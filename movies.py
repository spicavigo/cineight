...
class MovieDb:
    ...
    def browse(self, order_by='release', genres=None, min_votes=0, page=1, per_page=1000):
        url = config['urls']['movie.browse']  + "?order_by=%s&order=asc&min_votes=%d&page=%d&per_page=%d" % (order_by, min_votes, page, per_page)
    ...
    def max(self):
        ...
        etree = XmlHandler(url).getEt()
        return int(etree.find('movie').find('id').text)
